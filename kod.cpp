...
vector<vector<double>> matrixMultiply(const vector<vector<double>>& a, const vector<vector<double>>& b) {
    ...
    // Параллельное перемножение с использованием OpenMP
...
int main(int argc, char* argv[]) {
    ...
    timingFile << "Matrixsizexsize(OpenMP),Time(s),equalsround1e-6\n";
    timingFile.close();
    ...
    for (int size =10, i = 0; size <= 100; ++i) {
        ...
        string command = "python " + python_script_path + " mat1.txt mat2.txt result.txt check-result.txt";
        ...
        cout << "Time taken (OpenMP): " << time << " seconds" << endl;
